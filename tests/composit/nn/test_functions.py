...
@pytest.mark.parametrize("data_format", ["NCHW", "NHWC"])
def test_pool(composit_and_torch_functions, kernel_size, strides, padding, data_format):
    ...
    result_var = composit_function(
        image_arg, kernel_size=kernel_size, strides=strides, padding=padding, data_format=data_format
    )
