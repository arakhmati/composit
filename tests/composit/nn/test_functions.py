...
def test_convolution(strides, padding, data_layout):
    ...
    result_var = cnp.nn.convolution(image_arg, filters_arg, strides=strides, padding=padding, data_layout=data_layout)
...
@pytest.mark.parametrize("data_layout", ["NCHW", "NHWC"])
def test_pool(composit_and_torch_functions, kernel_size, strides, padding, data_layout):
    ...
    result_var = composit_function(
        image_arg, kernel_size=kernel_size, strides=strides, padding=padding, data_layout=data_layout
    )
