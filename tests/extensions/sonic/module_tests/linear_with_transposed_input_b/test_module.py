...
def create_parameters(k_size, n_size):
    parameters = {"weight": torch_random(n_size, k_size), "bias": torch_random(n_size)}
    return parameters


def torch_model(input_tensor, parameters):
    output_tensor = input_tensor @ parameters["weight"].T
    output_tensor = output_tensor + parameters["bias"]
...
def test_torch_vs_sonic(batch_size, m_size, k_size, n_size):
    hidden_states = torch_random(batch_size, m_size, k_size)
    parameters = create_parameters(k_size, n_size)
    ...
    encoder_output = torch_model(hidden_states, parameters)
    sonic_encoder_output = sonic_model(hidden_states, sonic_parameter_buffers)

    assert np.allclose(encoder_output.numpy(), sonic_encoder_output, atol=1e-4, rtol=1e-5)
...
def test_benchmark(benchmark, module, batch_size, m_size, k_size, n_size):
    ...
    if module == "torch":
        ...
        def function():
            hidden_states = torch_random(batch_size, m_size, k_size)
            torch_model(hidden_states, parameters)
