...
@wrap_as_instruction()
def interpolate(input_tensor, data_layout):
    if data_layout == "NHWC":
...
def convert_parameters_to_numpy(model, data_layout):
    ...
    for name, value in model.state_dict().items():
        ...
        if ("conv" in name and "weight" in name) and data_layout == "NHWC" and len(value.shape) == 4:
...
def attention_block(
    ...
    hidden_states = group_norm(
        ...
        group_norm_weight,
        group_norm_bias,
        channel_axis=-1 if data_layout == "NHWC" else 1,
        num_groups=32,
        epsilon=1e-6,
    )
...
def resnet_block(
    ...
    hidden_states = cnp.nn.convolution(hidden_states, conv_0_weight, padding=(1, 1), data_layout=data_layout)
    ...
    hidden_states = cnp.nn.convolution(hidden_states, conv_1_weight, padding=(1, 1), data_layout=data_layout)
    hidden_states += conv_1_bias

    if conv_shortcut_weight is not None:
        residual = cnp.nn.convolution(residual, conv_shortcut_weight, data_layout=data_layout)
        residual += conv_shortcut_bias

    return hidden_states + residual


def decoder(latents, parameters, *, data_layout):
    if data_layout == "NHWC":
        image = cnp.transpose(latents, (0, 2, 3, 1))

    output = cnp.nn.convolution(image, parameters["conv_in.weight"], padding=(1, 1), data_layout=data_layout)
    ...
    for up_block_index in range(4):
        ...
        if up_block_index <= 2:
            output = interpolate(output, data_layout=data_layout)
    ...
    output = cnp.nn.convolution(output, parameters["conv_out.weight"], padding=(1, 1), data_layout=data_layout)
